...
| 6 | `F9JM9CI6` | A | tagged (adjustments pending in Zotero): primary `ai-review` + `oversight-explanation`/`risk-routing`; facets → `intro-framing` + `counterpoint` + `general-code`, **remove `agentic`** (mode = neither; AI on review side only) | changelog §15: `counterpoint` role facet (stance polarity, direction-agnostic); `general-code` scope flag (object-of-oversight); mode-pair clarifier ("uses agents" ≠ agentic); narrowness ruled extent-not-tagged |
